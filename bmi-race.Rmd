...
# Plot the bar chart
barplot(bmi_counts, xlab = "BMI Category", ylab = "Number of Patients", main = "BMI Distribution",las = 1.5, cex.names = 0.6, ylim = c(0,4000),col = "skyblue", border = "black")
```
## Ethnicity
## MV HOUR 24
...
```{r}
vent_24_1 <- glm(vent24 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Underweight"), family = binomial)
...
rownames(out.mv.f) = c( 'ethnicityblack','genderF', 'age','admission_type',
...
```{r}
vent_24_2 <- glm(vent24 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Healthy weight"), family = binomial)
...
```{r}
vent_24_3 <- glm(vent24 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Overweight"), family = binomial)
...
```{r}
vent_24_4 <- glm(vent24 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Obese"), family = binomial)
...
```{r}
vent_24_5 <- glm(vent24 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Extremely obese"), family = binomial)
...
```{r}
df_mv24_1 <- total %>%
  group_by(bmi_c, ethnicity) %>%
  count()%>%
  pivot_wider(names_from = ethnicity, values_from = n)%>%
  rename(White = white, Black = black, "BMI" = bmi_c)
...
```{r}
df_mv24_2 <- total %>%
  group_by(bmi_c, ethnicity, vent24) %>%
  filter(vent24 == 0)%>%
  count() %>%
  pivot_wider(names_from = ethnicity, values_from = n) %>%
  rename(White = white, Black = black, "BMI" = bmi_c) %>%
  select("BMI", "White", "Black")
...
combined_df1$White <- paste(combined_df1$White.x, combined_df1$White.y, sep = "/")
combined_df1$Black <- paste(combined_df1$Black.x, combined_df1$Black.y, sep = "/")
...
p_values <- c(sum_vent24_1$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_vent24_2$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_vent24_3$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_vent24_4$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_vent24_5$coefficients["ethnicityblack", "Pr(>|z|)"])
...
table_mv_2 <- cbind(combined_df1,mv24_odd,p_values_df)
table_mv_2$'               '<- "               "
colnames(table_mv_2)[ncol(table_mv_2)] <- "               "
print(table_mv_2)
```
...
# 绘制森林图
p1 <- forest(table_mv_2[, c("BMI","White","Black","OR(95% CI)","P value",'               ')],   # 选择要在森林图中使用的数据列，这里包括变量名列、患者数量列、绘图要用的空白列和HR（95%CI）列
            est = table_mv_2$OddsRatio,          # 效应值，也就是HR列
            lower = table_mv_2$Lower_CI,  # 置信区间下限
            upper = table_mv_2$Upper_CI,  # 置信区间上限
            sizes = 0.2,        # 黑框框的大小
            ci_column = 6,             # 在第3列（可信区间列）绘制森林图
            ref_line = 1,              # 添加参考线
#            arrow_lab = c("Low risk", "High Risk"),  # 箭头标签，用来表示效应方向，如何设置取决于你的样本情况
            xlim = c(0,5),          # 设置x轴范围
            ticks_at = c(0,1,2,3,4),  # 在指定位置添加刻度
            theme = tm)               # 添加自定义主题
...
```{r}
icu_3_1 <- glm(icu3 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Underweight"), family = binomial)
...
```{r}
icu_3_2 <- glm(icu3 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Healthy weight"), family = binomial)
...
```{r}
icu_3_3 <- glm(icu3 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Overweight"), family = binomial)
...
```{r}
icu_3_4 <- glm(icu3 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Obese"), family = binomial)
...
```{r}
icu_3_5 <- glm(icu3 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Extremely obese"), family = binomial)
...
# Connect the corresponding numbers with "/"
combined_df2$White <- paste(combined_df2$White.x, combined_df2$White.y, sep = "/")
combined_df2$Black <- paste(combined_df2$Black.x, combined_df2$Black.y, sep = "/")
...
```{r}
# Extract the p-values for the "ethnicityblack" variable from each regression
p_values <- c(sum_icu3_1$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_icu3_2$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_icu3_3$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_icu3_4$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_icu3_5$coefficients["ethnicityblack", "Pr(>|z|)"])

# Create a dataframe with one column for the p-values
p_values_df <- data.frame(ifelse(p_values > 0.05, ">0.05", ifelse(p_values < 0.01, "<0.01", round(p_values, 3))))
colnames(p_values_df) <- "P value"

table_icu2 <- cbind(combined_df2,icu3_odd,p_values_df)
table_icu2$'               '<- "               "
colnames(table_icu2)[ncol(table_icu2)] <- "               "
print(table_icu2)
```
...
#                   arrow_type = "closed",    # 设置箭头类型为闭合箭头
                   ...
                   colnames_just = "center")   
# 绘制森林图
p2 <- forest(table_icu2[, c("BMI","White","Black","OR(95% CI)","P value",'               ')],   # 选择要在森林图中使用的数据列，这里包括变量名列、患者数量列、绘图要用的空白列和HR（95%CI）列
            est = table_icu2$OddsRatio,          # 效应值，也就是HR列
            lower = table_icu2$Lower_CI,  # 置信区间下限
            upper = table_icu2$Upper_CI,  # 置信区间上限
            sizes = 0.2,        # 黑框框的大小
            ci_column = 6,             # 在第3列（可信区间列）绘制森林图
            ref_line = 1,              # 添加参考线
#            arrow_lab = c("Low risk", "High Risk"),  # 箭头标签，用来表示效应方向，如何设置取决于你的样本情况
...
```{r}
hos_7_1 <- glm(hos7 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Underweight"), family = binomial)
...
```{r}
hos_7_2 <- glm(hos7 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Healthy weight"), family = binomial)
...
```{r}
hos_7_3 <- glm(hos7 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Overweight"), family = binomial)
...
```{r}
hos_7_4 <- glm(hos7 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Obese"), family = binomial)
...
```{r}
hos_7_5 <- glm(hos7 ~ ethnicity+gender+age+admission_type+charlson_comorbidity_index +cabg+valve_surgery+sofa, data =subset(total, bmi_c == "Extremely obese"), family = binomial)
...
# Connect the corresponding numbers with "/"
combined_df3$White <- paste(combined_df3$White.x, combined_df3$White.y, sep = "/")
combined_df3$Black <- paste(combined_df3$Black.x, combined_df3$Black.y, sep = "/")
...
```{r}
# Extract the p-values for the "ethnicityblack" variable from each regression
p_values <- c(sum_hos7_1$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_hos7_2$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_hos7_3$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_hos7_4$coefficients["ethnicityblack", "Pr(>|z|)"],
              sum_hos7_5$coefficients["ethnicityblack", "Pr(>|z|)"])

# Create a dataframe with one column for the p-values
p_values_df <- data.frame(ifelse(p_values > 0.05, ">0.05", ifelse(p_values < 0.01, "<0.01", round(p_values, 3))))
colnames(p_values_df) <- "P value"

table_hos7 <- cbind(combined_df3,hos7_odd,p_values_df)
table_hos7$'               '<- "               "
colnames(table_hos7)[ncol(table_hos7)] <- "               "
print(table_hos7)
```
...
#                   arrow_type = "closed",    # 设置箭头类型为闭合箭头
                   ...
                   colnames_just = "center")   
# 绘制森林图
p3 <- forest(table_hos7[, c("BMI","White","Black","OR(95% CI)","P value",'               ')],   # 选择要在森林图中使用的数据列，这里包括变量名列、患者数量列、绘图要用的空白列和HR（95%CI）列
            est = table_hos7$OddsRatio,          # 效应值，也就是HR列
            lower = table_hos7$Lower_CI,  # 置信区间下限
            upper = table_hos7$Upper_CI,  # 置信区间上限
            sizes = 0.2,        # 黑框框的大小
            ci_column = 6,             # 在第3列（可信区间列）绘制森林图
            ref_line = 1,              # 添加参考线
#            arrow_lab = c("Low risk", "High Risk"),  # 箭头标签，用来表示效应方向，如何设置取决于你的样本情况
            xlim = c(0,8),          # 设置x轴范围
            ticks_at = c(0,2,4,6,8),  # 在指定位置添加刻度
            theme = tm)               # 添加自定义主题
#            footnote = "This is the demo data. Please feel free to change\nanything you want.")  # 添加脚注信息
p3
```
...
```{r}
ggsave("mv24_bmi_race.png", plot = p1)
ggsave("icu3_bmi_race.png", plot = p2)
ggsave("hos7_bmi_race.png", plot = p3)
```
